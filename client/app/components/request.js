...
let ruleDataProcess = function (vm, params) {
  ...
  if (params.isGreaterThanLimit) {
    angular.forEach(params.discountRanges, function (item) {
      item.greaterThan = Math.round(item.greaterThan * 100);
      item.discount = Math.round(item.discount * 100);
      item.isGreaterThanLimit = params.isGreaterThanLimit
    });
  }
  if (!params.isGreaterThanLimit) {
    params.discountRanges.discount *= 100;
    params.discountRanges.isGreaterThanLimit = params.isGreaterThanLimit;
  }
